    ...
    public static class ServiceFactory
    {
        ...
        private static readonly Dictionary<Type, Func<UnitOfWork, IService>> _Services =
            new Dictionary<Type, Func<UnitOfWork, IService>> {
                  ...
                  { typeof(IAddressService), (uow) => { return new AddressService(uow); ; } },
            { typeof(ICompanyService), (uow) => { return new CompanyService(uow); ; } },
              { typeof(IContactService), (uow) => { return new ContactService(uow); ; } },
                { typeof(IDropDownListService), (uow) => { return new DropDownListService(uow); ; } },
                { typeof(ITelephoneService), (uow) => { return new TelephoneService(uow); ; } }
            };
